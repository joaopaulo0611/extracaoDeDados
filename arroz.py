...
class ArrozSpider(scrapy.Spider):
    name = "arroz"
    ...
    def update_settings(cls, settings):
        ...
        settings.set("BOT_NAME", "Pesquisa_Arroz_Giassi", priority="spider")
        settings.set("ITEM_PIPELINES", {"pipelines.SQLAlchemyPipeline": 300}, priority="spider")

    def parse(self, response: scrapy.http.Response):
        response.selector.remove_namespaces()
        produtos_arroz = response.xpath('//url/loc[contains(text(),"arroz") and contains(text(),"5kg")]/text()').getall()

        print(f'Encontrados {len(produtos_arroz)} produtos de arroz 5kg neste sitemap')

        for url in produtos_arroz:
