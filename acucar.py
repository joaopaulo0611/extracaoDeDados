...
class AcucarSpider(scrapy.Spider):
    name = "acucar"
    ...
    def update_settings(cls, settings):
        ...
        settings.set("BOT_NAME", "Pesquisa_Acucar_Giassi", priority="spider")
        settings.set("ITEM_PIPELINES", {"pipelines.SQLAlchemyPipeline": 300}, priority="spider")

    def parse(self, response: scrapy.http.Response):
        response.selector.remove_namespaces()
        produtos_acucar = response.xpath('//url/loc[contains(text(),"acucar") and contains(text(),"1kg")]/text()').getall()

        print(f'Encontrados {len(produtos_acucar)} produtos de açúcar 1kg neste sitemap')

        for url in produtos_acucar:
    ...
    def parse_produto(self, response: scrapy.http.Response):
        ...
        # Tenta extrair preço do JSON-LD ou meta tags
