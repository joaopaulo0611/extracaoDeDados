...
class FeijaoSpider(scrapy.Spider):
    ...
    def update_settings(cls, settings):
        ...
        settings.set("BOT_NAME", "Pesquisa_Feijao_Giassi", priority="spider")
        settings.set("ITEM_PIPELINES", {"pipelines.SQLAlchemyPipeline": 300}, priority="spider")

    def parse(self, response: scrapy.http.Response):
        response.selector.remove_namespaces()
        produtos_feijao = response.xpath('//url/loc[contains(text(),"feijao") and contains(text(),"1kg")]/text()').getall()
        print(f'Encontrados {len(produtos_feijao)} produtos de feijão 1kg neste sitemap')

        for url in produtos_feijao:
